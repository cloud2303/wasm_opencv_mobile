  ...
  return async function (moduleArg = {}) {
    ...
    var window = wx;
      ...
      function findWasmBinary() {
          return locateFile("wasm_test.wasm")
      }
      ...
      async function instantiateArrayBuffer(binaryFile, imports) {
          try {
              ...
              var instance = await WebAssembly.instantiate("/lib/wasm_test.wasm", imports);
              return instance
          } catch (reason) {
      ...
      function createJsInvoker(argTypes, isClassMethodFunc, returns, isAsync) {
        ...
        // 静态函数实现
        function staticInvoker() {
          var humanName = arguments[0];
          var throwBindingError = arguments[1];
          var invoker = arguments[2];
          var fn = arguments[3];
          var runDestructors = arguments[4];
          var fromRetWire = arguments[5];
          var toClassParamWire = arguments[6];

          // 获取参数转换函数
          var argConverters = [];
          for (var i = 7; i < arguments.length; i++) {
            argConverters.push(arguments[i]);
          }
          ...
          return function () {
            ...
            for (var i = 0; i < argCount; i++) {
              var converter = argConverters[i];
              var wiredArg = converter
                ? converter(destructors, arguments[i])
                : arguments[i];
              args.push(wiredArg);
            }
            ...
            var rv;
            if (returns || isAsync) {
              rv = invoker.apply(null, args);
            } else {
              invoker.apply(null, args);
            }

            // 清理析构函数
            if (needsDestructorStack && destructors) {
              runDestructors(destructors);
            } else {
              // 手动清理非堆栈析构函数
              var argIndex = 0;
              if (isClassMethodFunc) {
                var thisDestructor = argConverters[argCount]; // 获取this的析构函数
                if (thisDestructor && argTypes[1].destructorFunction !== null) {
                  thisDestructor(thisWired);
                }
                argIndex = 1;
              }
              ...
              for (var i = 2; i < argTypes.length; i++) {
                if (argTypes[i].destructorFunction !== null) {
                  var destructorIndex = argCount + argIndex;
                  var destructor = argConverters[destructorIndex];
                  if (destructor) {
                    destructor(args[i - (isClassMethodFunc ? 0 : 1)]);
                  }
                  argIndex++;
                }
              }
      ...
      var __emval_create_invoker = (argCount, argTypesPtr, kind) => {
        ...
        // 静态调用器函数
        function staticInvoker(handle, methodName, destructorsRef, args) {
          var toValue = Emval.toValue;
          var target;
          var result;
          ...
          switch (kind) {
            ...
            case 3: // 空调用
              ...
              break;
            case 1: // 方法调用
              var obj = toValue(handle);
              var methodKey = getStringOrSymbol(methodName);
              target = obj[methodKey];
              break;
          }

          // 提取参数
          var callArgs = [];
          for (var i = 0; i < argFromPtr.length; i++) {
            var argReader = argFromPtr[i];
            var argOffset = i * GenericWireTypeSize;
            callArgs.push(argReader(args + argOffset));
          }

          // 执行调用
          if (kind === 0) {
            // 直接函数调用
            result = target.apply(null, callArgs);
          } else if (kind === 1) {
            // 方法调用
            result = target.apply(toValue(handle), callArgs);
          } else if (kind === 2) {
            // 构造函数调用
            result = new (target.bind.apply(target, [null].concat(callArgs)))();
          } else if (kind === 3) {
            // 空调用，相当于直接执行参数表达式
            // 原始代码是 functionBody = ""; functionBody += `(${args})`
            // 这意味着是 (arg0, arg1, ...)，在JavaScript中这是逗号运算符，返回最后一个值
            result =
              callArgs.length > 0 ? callArgs[callArgs.length - 1] : undefined;
          }
        ...
        return emval_addMethodCaller(
          createNamedFunction(functionName, staticInvoker)
        );
      };
      ...
      function assignWasmExports(wasmExports) {
          ___getTypeName = wasmExports["G"];
          _malloc = wasmExports["I"];
          _free = wasmExports["J"]
      }
...
} else if (typeof define === "function" && define["amd"]) define([], () => createModule);
